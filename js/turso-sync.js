/* ============================================================
   ADEPTIO · v2.3.2.db — Turso hybrid sync (offline-first)
   Blueprint v2.3.2 §02/§05 made real: localStorage remains the
   working cache (instant UI, works offline / file://), Turso is
   the durable cloud copy. One database, ten table groups —
   tables are prefixed per store (people_employees, time_punches,
   audit_events, …) so per-store restore keeps its blast radius.
...
window.TURSO = (function () {
  ...
  function pushStmts(storeId, t) {
    ...
    cat.tables.forEach(tb => {
      ...
      const rows = Array.isArray(tables[tb]) ? tables[tb] : [];
